...
fun interface AutoScrollCondition {
     ...
     */
    fun shouldScroll(oldModel: CartesianChartModel?, newModel: CartesianChartModel): Boolean

    companion object {
        /** Prevents any automatic scrolling from occurring. */
        val Never: AutoScrollCondition = AutoScrollCondition { _, _ -> false }

        /**
         * Triggers an automatic scroll when the size of the model increases (that is, the contents of
         * the chart become wider).
         */
        val OnModelGrowth: AutoScrollCondition = AutoScrollCondition { oldModel, newModel ->
            oldModel != null &&

...
import com.telekom.odsystem.charts.core.common.isBoundOf
import com.telekom.odsystem.charts.core.common.orZero
import kotlin.math.ceil
import kotlin.math.max
import kotlin.math.min
import com.telekom.odsystem.charts.core.cartesian.getFullXRange as internalGetFullXRange
...
protected constructor(
    ...
    override fun drawUnderLayers(context: CartesianDrawingContext) {
        with(context) {
            ...
            labelValues.forEachIndexed { index, x ->
                ...
                val nextX =
                    labelValues.getOrNull(index + 1) ?: (fullXRange.endInclusive.doubled - x)
                val maxWidth =
                    ceil(
                        min(
                            x - previousX,
                            nextX - x
                        ) / ranges.xStep * layerDimensions.xSpacing
                    ).toInt()
    ...
    override fun updateLayerDimensions(
        ...
        val maxLabelWidth =
            context.getMaxLabelWidth(
                layerDimensions,
                context.internalGetFullXRange(layerDimensions)
            )

...
import com.telekom.odsystem.charts.core.cartesian.data.getSliceIndices
import com.telekom.odsystem.charts.core.cartesian.getVisibleXRange
...
import com.telekom.odsystem.charts.core.cartesian.marker.MutableColumnCartesianLayerMarkerTarget
...
import com.telekom.odsystem.charts.core.common.extractColor
...
import com.telekom.odsystem.charts.core.common.unaryMinus
import getRepeating
...
protected constructor(
    ...
    protected open fun CartesianDrawingContext.drawChartInternal(
        ...
        model.series.forEachIndexed { index, entryCollection ->
            ...
            entryCollection.subList(firstVisibleIndex, lastVisibleIndex + 1).forEach { entry ->
                ...
                height =
                    (columnInfo?.height
                        ?: (abs(entry.y) / yRange.length)).toFloat() * layerBounds.height()
    ...
    protected open fun CartesianDrawingContext.drawDataLabel(
        ...
        dataLabel?.let { textComponent ->
            ...
            var maxWidth =
                when {
                    ...
                    mergeMode is MergeMode.Grouped ->
                        (columnThicknessDp + min(
                            columnCollectionSpacingDp,
                            mergeMode.columnSpacingDp
                        ).half)
                            .pixels * zoom
            ...
            val labelVerticalPosition =
                if (dataLabelValue < 0f) -dataLabelPosition else dataLabelPosition
    ...
    protected open fun CartesianDrawingContext.updateMarkerTargets(
        ...
        val targetColumn =
            ColumnCartesianLayerMarkerTarget.Column(entry, limitedCanvasY, markerColor)
        when (mergeMode) {
            is MergeMode.Grouped ->
        ...
        when (mergeMode) {
            is MergeMode.Stacked ->
                (0..<entryCollectionSize)
                    .maxOf { seriesIndex ->
                        columnProvider.getWidestSeriesColumn(
                            seriesIndex,
                            model.extraStore
                        ).thicknessDp
                    }
    ...
    protected open fun CartesianDrawingContext.getDrawingStart(
        ...
        return layerBounds.getStart(isLtr) +
                (layerDimensions.startPadding +
                        (mergeModeComponent - getColumnCollectionWidth(
    ...
    protected open fun CartesianMeasuringContext.getCumulatedThickness(count: Int): Float {
        ...
        for (seriesIndex in 0..<count) {
            thickness += columnProvider.getWidestSeriesColumn(
                seriesIndex,
                model.extraStore
            ).thicknessDp
        }
